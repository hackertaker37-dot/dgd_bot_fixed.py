# ======================================================================================
# 𝙓𝙒𝘿 𝙎𝙈𝙎 - النسخة النهائية العملاقة (جميع الأزرار ولوحة التحكم شغالة 100%)
# المطور: hacker Taker
# ======================================================================================
...
# ======================================================================================
# إعدادات الموقع XWD (حل مشكلة 443)
# ======================================================================================
XWD_API_KEY = "4886d4297bcfb669bf3b3d2d8d1c4ee2"
...
def init_db():
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT, last_name TEXT, country_code TEXT, assigned_number TEXT, is_banned INTEGER DEFAULT 0, private_combo_country TEXT DEFAULT NULL)''')
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS private_combos (user_id INTEGER, country_code TEXT, range TEXT, PRIMARY KEY (user_id, country_code))''')
        c.execute('''CREATE TABLE IF NOT EXISTS force_sub_channels (id INTEGER PRIMARY KEY AUTOINCREMENT, channel_url TEXT UNIQUE NOT NULL, description TEXT DEFAULT '', enabled INTEGER DEFAULT 1)''')
...
def save_user(user_id, username="", first_name="", last_name="", country_code=None, assigned_number=None, private_combo_country=None, is_banned=None):
    try:
        ...
        c.execute("REPLACE INTO users (user_id, username, first_name, last_name, country_code, assigned_number, is_banned, private_combo_country) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (user_id, username, first_name, last_name, country_code, assigned_number, is_banned, private_combo_country))
        conn.commit(); conn.close()
    except: pass
...
def save_combo(country_code, range_str, user_id=None):
    try:
        conn = sqlite3.connect(DB_PATH); c = conn.cursor()
        if user_id: c.execute("REPLACE INTO private_combos (user_id, country_code, range) VALUES (?, ?, ?)", (user_id, country_code, range_str))
        else:
...
def add_force_sub_channel(url, desc=""):
    try: conn = sqlite3.connect(DB_PATH); c = conn.cursor(); c.execute("INSERT INTO force_sub_channels (channel_url, description, enabled) VALUES (?, ?, 1)", (url.strip(), desc.strip())); conn.commit(); conn.close(); return True
    except: return False

def delete_force_sub_channel(channel_id):
    try: conn = sqlite3.connect(DB_PATH); c = conn.cursor(); c.execute("DELETE FROM force_sub_channels WHERE id=?", (channel_id,)); res = c.rowcount > 0; conn.commit(); conn.close(); return res
    except: return False

def toggle_force_sub_channel(channel_id):
    try: conn = sqlite3.connect(DB_PATH); c = conn.cursor(); c.execute("UPDATE force_sub_channels SET enabled = 1 - enabled WHERE id=?", (channel_id,)); conn.commit(); conn.close()
    except: pass
...
def force_sub_markup():
    ...
    for _, url, desc in channels: markup.add(types.InlineKeyboardButton(f"📢 {desc}" if desc else "📢 اشترك", url=url))
    markup.add(types.InlineKeyboardButton("✅ تحقق من الاشتراك", callback_data="check_sub"))
    return markup

# ======================================================================================
# دوال الاتصال بـ XWD API (معالجة 403 و 443)
# ======================================================================================
def xwd_get_number(range_str):
    ...
    try:
        ...
        if resp.status_code == 403:
            ...
            except: msg = "الخادم رفض الطلب (403)"
...
def xwd_get_balance():
    ...
    try:
        ...
        if resp.status_code == 403: raise Exception("الرصيد مرفوض (403). قد يكون المفتاح محظوراً أو الرصيد صفر.")
        if resp.status_code != 200: raise Exception("تعذر جلب الرصيد")
        data = resp.json()
        if data.get("success"): return data.get("balance", 0.0)
        else: return 0.0
    except Exception as e: logger.error(f"XWD balance error: {e}"); raise
...
# ======================================================================================
# لوحة المفاتيح الرئيسية (الأزرار)
# ======================================================================================
def main_keyboard():
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    markup.add(types.KeyboardButton("📱 الحصول على رقم"), types.KeyboardButton("💰 الرصيد"))
    markup.add(types.KeyboardButton("🔄 تبديل الرقم"), types.KeyboardButton("📊 الإحصائيات"))
    markup.add(types.KeyboardButton("🤝 شارك واربح"), types.KeyboardButton("🔗 جروب البوت"))
    return markup

# ======================================================================================
# حلقة جلب OTP الخلفية (كل 3 ثوانٍ)
# ======================================================================================
def main_loop():
    sent_ids = set()
    ...
    while True:
        try:
            ...
            time.sleep(3)
        except Exception as e: logger.error(f"Main loop error: {e}"); time.sleep(5)

# ======================================================================================
# أزرار التفاعل (Inline Callbacks)
# ======================================================================================
...
def check_sub(c):
    if force_sub_check(c.from_user.id):
        bot.answer_callback_query(c.id, "✅ تم التحقق!", show_alert=True)
        start_h(c.message)
    else:
        bot.answer_callback_query(c.id, "❌ لم تشترك!", show_alert=True)
...
def country_h(c):
    ...
    try:
        ...
        try:
            ...
            markup.row(types.InlineKeyboardButton("🔄 تغيير", callback_data=f"country_{code}"), types.InlineKeyboardButton("🌍 تغيير الدولة", callback_data="back"))
            markup.row(types.InlineKeyboardButton("👥 جروب", url="https://t.me/numhj"), types.InlineKeyboardButton("🏠 القائمة", callback_data="back"))
...
def start_h(m):
    ...
    if not force_sub_check(user_id):
        ...
        if markup: bot.send_message(m.chat.id, "🔒 اشترك في القنوات أولاً.", reply_markup=markup)
        return
...
def balance_handler(m):
    try:
        ...
        bot.reply_to(m, f"💰 <b>الرصيد الحالي في الموقع:</b>\n<code>{bal_str} دولار</code>", parse_mode="HTML")
    except Exception as e: bot.reply_to(m, f"❌ تعذر جلب الرصيد:\n{str(e)}")
...
def change_num_handler(m):
    ...
    if user and user[5]:
        ...
        if code in AVAILABLE_COUNTRIES:
            ...
            country_h(c)
        else: bot.reply_to(m, "⚠️ لم يتم العثور على دولة مسجلة، اختر دولة جديدة أولاً.")
    else: bot.reply_to(m, "⚠️ ليس لديك رقم مخصص حالياً.")

@bot.message_handler(func=lambda m: m.text == "📊 الإحصائيات")
def stats_handler(m):
    users = len(get_all_users()); active = len(get_active_numbers())
    bot.reply_to(m, f"📊 <b>إحصائيات البوت:</b>\n👥 عدد المستخدمين: {users}\n📱 أرقام نشطة حالياً: {active}", parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text == "🤝 شارك واربح")
def refer_handler(m):
    bot_username = "Taker_OTP_BOT"
    bot.reply_to(m, f"🤝 <b>شارك واربح!</b>\n\nرابط الدعوة الخاص بك:\n<code>https://t.me/{bot_username}?start={m.from_user.id}</code>", parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text == "🔗 جروب البوت")
def group_handler(m): bot.reply_to(m, "🔗 <b>جروب البوت:</b>\nhttps://t.me/numhj", parse_mode="HTML")
...
def admin_panel(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id, "⚠️ غير مسموح.", show_alert=True)
    markup = types.InlineKeyboardMarkup()
...
def add_combo_country(m):
    code = m.text.strip()
    if code not in AVAILABLE_COUNTRIES: return bot.reply_to(m, "❌ كود غير مدعوم!")
...
def add_combo_range(m):
    data = user_states[m.from_user.id]; code = data["code"]; range_str = m.text.strip()
    save_combo(code, range_str); bot.reply_to(m, f"✅ تم إضافة الرينج {range_str}"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_del_combo")
def admin_del_combo(c):
    if not is_admin(c.from_user.id): return
    combos = get_all_combos()
    if not combos: return bot.answer_callback_query(c.id, "لا توجد رينجات!", show_alert=True)
    markup = types.InlineKeyboardMarkup()
...
def del_combo_exec(c):
    parts = c.data.split("_"); code, idx = parts[2], int(parts[3])
    if delete_combo(code, idx): bot.answer_callback_query(c.id, "✅ تم الحذف", show_alert=True)
    else: bot.answer_callback_query(c.id, "❌ فشل", show_alert=True)
    admin_del_combo(c)
...
def admin_full_report(c): 
    if not is_admin(c.from_user.id): return
    with open(DB_PATH, "rb") as f: bot.send_document(c.message.chat.id, f, caption="📄 تقرير شامل")
...
def broadcast_all_send(m):
    ...
    bot.reply_to(m, f"✅ تم الإرسال إلى {count} مستخدم"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_broadcast_user")
def broadcast_user(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "broadcast_user_id"; bot.edit_message_text("📨 أدخل معرف المستخدم:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "broadcast_user_id")
def broadcast_user_id(m):
    try: uid = int(m.text); user_states[m.from_user.id] = f"broadcast_msg_{uid}"; bot.reply_to(m, "📨 أرسل الرسالة:")
...
def broadcast_user_send(m):
    ...
    try: bot.copy_message(uid, m.chat.id, m.message_id); bot.reply_to(m, f"✅ تم الإرسال للمستخدم {uid}")
    except: bot.reply_to(m, "❌ فشل"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_ban")
def admin_ban(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "ban"; bot.edit_message_text("🚫 أرسل ID المستخدم للحظر:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "ban")
def ban_exec(m):
    try: uid = int(m.text); save_user(uid, is_banned=1); bot.reply_to(m, f"✅ تم حظر {uid}")
    except: bot.reply_to(m, "❌ معرف غير صحيح"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_unban")
def admin_unban(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "unban"; bot.edit_message_text("✅ أرسل ID المستخدم لفك الحظر:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "unban")
def unban_exec(m):
    try: uid = int(m.text); save_user(uid, is_banned=0); bot.reply_to(m, f"✅ تم فك حظر {uid}")
    except: bot.reply_to(m, "❌ معرف غير صحيح"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_user_info")
def admin_info(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "info"; bot.edit_message_text("👤 أرسل ID المستخدم:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "info")
def info_exec(m):
    try:
        uid = int(m.text); user = get_user(uid)
        if user: bot.reply_to(m, f"👤 {user[0]}\n@{user[1] or 'N/A'}\nالرقم: {user[4] or 'لا يوجد'}\nمحظور: {'نعم' if user[6] else 'لا'}")
        else: bot.reply_to(m, "❌ غير موجود")
    except: bot.reply_to(m, "❌ معرف غير صحيح"); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_set_welcome_photo")
def set_photo_req(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "wait_photo"; bot.edit_message_text("🖼️ أرسل صورة الترحيب:", c.message.chat.id, c.message.message_id)

@bot.message_handler(content_types=['photo'], func=lambda m: user_states.get(m.from_user.id) == "wait_photo")
def set_photo_exec(m): set_setting("welcome_photo", m.photo[-1].file_id); bot.reply_to(m, "✅ تم حفظ صورة الترحيب."); del user_states[m.from_user.id]

@bot.callback_query_handler(func=lambda c: c.data == "admin_del_welcome_photo")
def del_photo_req(c): set_setting("welcome_photo", ""); bot.answer_callback_query(c.id, "🗑️ تم حذف الصورة", show_alert=True); admin_panel(c)
...
def add_private(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "add_private_user"; bot.edit_message_text("➕ أدخل معرف المستخدم:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "add_private_user")
def add_private_user(m):
    try: uid = int(m.text); user_states[m.from_user.id] = f"add_private_country_{uid}"
    except: return bot.reply_to(m, "❌ معرف غير صحيح")
    markup = types.InlineKeyboardMarkup(row_width=2)
...
def select_private(c):
    parts = c.data.split("_"); uid = int(parts[2]); code = parts[3]
    save_user(uid, private_combo_country=code); bot.answer_callback_query(c.id, "✅ تم تعيين رينج خاص", show_alert=True); private_combo(c)

@bot.callback_query_handler(func=lambda c: c.data == "del_private_combo")
def del_private(c):
    if not is_admin(c.from_user.id): return
    user_states[c.from_user.id] = "del_private_user"; bot.edit_message_text("🗑️ أدخل معرف المستخدم:", c.message.chat.id, c.message.message_id)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id) == "del_private_user")
def del_private_user(m):
    try: uid = int(m.text); save_user(uid, private_combo_country=None); bot.reply_to(m, f"✅ تم مسح الرينج الخاص")
    except: bot.reply_to(m, "❌ معرف غير صحيح"); del user_states[m.from_user.id]
...
# ======================================================================================
# خادم الويب (Flask) لتشغيل Render
# ======================================================================================
app = Flask(__name__)
